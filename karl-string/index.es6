let stringExtend = (funcName, func)=> {
    if (!String.prototype[funcName]) {
        String.prototype[funcName] = func;
    }
};

stringExtend("includes", (search, start)=> {
...
stringExtend("urlEncode", function () {
    let output = encodeURIComponent(this);
    return output;
});

stringExtend("urlDecode", function () {
    let output = decodeURIComponent(this);
    return output;
});
...
stringExtend("base64UrlEncode", function () {
    let output = this.base64Encode().urlEncode();
    return output;
});

stringExtend("urlBase64Decode", function () {
    let output = this.urlDecode().base64Decode();
    return output;
});
